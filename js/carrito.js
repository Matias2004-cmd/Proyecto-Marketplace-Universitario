...
const cargarCarrito = () => {
    ...
    } catch (error) { // si el dato guardado esta corrupto, se reinicia el carrito. 
        carrito = []
    }
};
...
const agregarAlCarrito = (id) => {
    ...
    const itemExistente = carrito.find(
        (item) => item.productoId === Number(id)
    );
    if (itemExistente) {
...
const renderCarrito = () => {
    ...
    carrito.forEach((item) => {
        ...
        const botonRestar = crearBoton("−", "btn btn-outline-secondary btn-sm", `Restar una unidad de ${producto.nombre}`, () => cambiarCantidad(producto.id, -1));
        const cantidad = document.createElement("span");
        cantidad.className = "fw-bold";
        cantidad.textContent = item.cantidad;
        const botonSumar = crearBoton("+", "btn btn-outline-primary btn-sm", `Sumar una unidad de ${producto.nombre}`, () => cambiarCantidad(producto.id, 1));
        const botonQuitar = crearBoton("Quitar", "btn btn-outline-danger btn-sm", `Quitar ${producto.nombre} del carrito`, () => quitarDelCarrito(producto.id));
